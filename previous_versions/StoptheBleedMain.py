...
api_link = "http://tosmcoe0005.ttu.edu:3000"
timelist = []
pressurelist = []
ma_xlist = []
blood_loss = []
...
def end(event4):
    selected_user.training_data['ma_xlist'] = ma_xlist.copy()
    selected_user.training_data['x_list'] = timelist.copy()
    selected_user.training_data['y_list'] = pressurelist.copy()
    selected_user.training_data['blood_loss'] = blood_loss.copy()
    make_http_put_request(event4.state)
    #print('View end')    
    if event4.state == 0:
        View_Fail()
    else:
...
def make_http_get_request(trainingKey):
    # Function to make an HTTP request
    url = f"{api_link}/classUsers"
    ...
    try:
        ...
        current_class = TrainingClass(class_id=class_id, users=users_list)

        print(current_class.class_id)
        for user in current_class.users:
...
def make_http_put_request(success: bool):
    ...
    data =  {'class_id': current_class.class_id, 'user_id': selected_user.user_id, 'x_list': timelist, 'y_list': pressurelist, 'bloodloss': blood_loss, 'type': trainingType, 'passed': success}
...
def display_window():
    ...
    if window == 3:
        def are_choices_selected():
            return wound.get() and sound.get() and blood.get()

        # Update the state of the "Begin" button based on choices
        def update_begin_button_state():
            if are_choices_selected():
                button_scenario['state'] = 'normal'
            else:
                button_scenario['state'] = 'disabled'
        ...
        # Bind the function to the choices variables
        wound.trace_add('write', lambda *args: update_begin_button_state())
        sound.trace_add('write', lambda *args: update_begin_button_state())
        blood.trace_add('write', lambda *args: update_begin_button_state())
        ...
        button_scenario = Button(frame, text="Begin", command=lambda: [open_class_window()], font=("Arial", largetitletext), bg="firebrick3", fg="white", state='disabled')
       ...
       # Add the frame hat the graph will go in
        ...
        ##print ('blood', blood)      
        #Add Title frame        
        button_home = Button(frame, text="Select\nNew User", command = lambda: [open_class_window()],bg="firebrick3",fg="white", font=("Arial", largetext))
        ...
        def update_button_states():
            ...
            if selected_index:
                selected_user = current_class.users[selected_index[0]]
                button_start['state'] = 'normal'
                button_view_graph['state'] = 'normal' if selected_user.training_data['x_list'] else 'disabled'
            else:
        ...
        def request():
            make_http_get_request(input_entry.get())
            populate_listbox()
        ...
        def view_graph():
            ...
            if selected_index:
                ...
                timelist = current_class.users[selected_index[0]].training_data['x_list'].copy()
                pressurelist = current_class.users[selected_index[0]].training_data['y_list'].copy()
                ma_xlist = current_class.users[selected_index[0]].training_data['ma_xlist'].copy()
                View_Graph()
        ...
        button_back = Button(frame, text="Go Back", command=lambda: [open_scenario_window()], bg="firebrick3", fg="white", font=("Arial", largetext))
...
#Calculate Blood Loss-------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if BloodLost >= 3: #total blood lost to shut off machine, default is 3 (liters)
            ...
            root.event_generate("<<event4>>", state=str(0))
            sleep(5)
