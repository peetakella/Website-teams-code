...
api_link = "http://10.187.243.199:3000"
timelist = []
pressurelist = []
ma_xlist = []
blood_loss = []
...
def end(event4):
    selected_user.training_data['ma_xlist'] = ma_xlist
    selected_user.training_data['x_list'] = timelist
    selected_user.training_data['y_list'] = pressurelist
    selected_user.training_data['blood_loss'] = blood_loss
...
def display_window():
    ...
    if window == 3:
        ...
        # TODO: Detmine if this is required if using the ttk library
        '''#Add Title frame        
        frame1 = LabelFrame(frame, pady=25, fg="black", bg="gray75")
        frame1.place(x=.025*w, y=.025*h, height=.2*h, width=.95*w)
       ...
       # Add the frame hat the graph will go in
        ...
        def view_graph():
            ...
            if selected_index:
                ...
                timelist = (current_class.users[selected_index[0]].training_data['x_list'])
                pressurelist = (current_class.users[selected_index[0]].training_data['y_list'])
                ma_xlist = (current_class.users[selected_index[0]].training_data['ma_xlist'])
                View_Graph()
